...
**인지도 있는 언론사 기사만 보고 싶어요 / 모든 언론사를 다 포함하고 싶어요.**
기본값은 `PRESS_MAP`에 등록된 언론사의 기사만 채택하도록 되어 있습니다 (`ONLY_KNOWN_PRESS = true`). 특정 언론사를 더 포함하고 싶으면 `PRESS_MAP`에 도메인을 추가하면 되고, 언론사 제한 없이 모두 포함하고 싶으면 `ONLY_KNOWN_PRESS`를 `false`로 바꾸면 됩니다.

**여러 언론사가 같은 소식을 보도하면 하나만 남기고 싶어요.**
`DEDUPE_SIMILAR_ARTICLES = true`(기본값)이면, 제목+본문 유사도가 `DUPLICATE_SIMILARITY_THRESHOLD`(기본 0.5, 즉 50%) 이상인 기사들 중 `PRESS_MAP`에 먼저 나열된(우선순위 높은) 언론사 기사 하나만 남기고 나머지는 수집하지 않습니다. 언론사 우선순위를 바꾸고 싶으면 `PRESS_MAP`에 등록된 순서를 바꾸면 되고, 중복 제거 자체를 끄고 싶으면 `DEDUPE_SIMILAR_ARTICLES`를 `false`로 바꾸면 됩니다. 실제 결과를 보고 너무 많이/적게 걸러진다면 `DUPLICATE_SIMILARITY_THRESHOLD` 숫자를 조정해보세요(낮출수록 더 많이 중복으로 판단).
